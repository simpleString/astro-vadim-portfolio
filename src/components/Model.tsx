...
const Model = ({ modelName }: { modelName: string }) => {
  ...
  const [isShowZoomButton, setIsShowZoomButton] = useState(false);
  const [showModal, setShoModal] = useState(false);

  const hideZoomButton = useCallback(() => {
    setIsShowZoomButton(false);
  }, []);

  const showZoomButton = useCallback(() => {
    setIsShowZoomButton(true);
  }, []);

  const showModalZoom = useCallback(() => {
    setShoModal(true);
  }, []);
  ...
  return (
    <>
      <div
        ...
        ref={refContainer}
        onMouseOver={showZoomButton}
        onMouseLeave={hideZoomButton}
      >
        {<SpinnerCircular className="w-full h-full m-auto" enabled={loading} />}
        {/* {isShowZoomButton && (
          <button
            onClick={showModalZoom}
            className="btn absolute mx-auto bottom-4 block w-full"
          >
